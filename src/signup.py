...
class Signup:
    ...
    def _get_csrf(self):
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"fail to get csrf")
        raise Exception(
            f"fail to get csrf ,please check your network or ip"
        )
    ...
    def _get_authorized_url(self, csrf):
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"fail to get authorized url ")

        raise Exception(
            f"fail to get authorized url,please check your network or ip"
        )
    ...
    def _get_state(self, authorized_url):
        ...
        try:
            ...
            logger.warning(f"fail to get state {e} ")

        raise Exception(
            f"fail to get state, please check your network or ip"
        )
    ...
    def _check_identifier(self, state, identifier):
        ...
        try:
            ...
            logger.warning(f"fail to check identifier , {e}")


        raise Exception(
            f"fail to check identifier, please check your network or ip"
        )
    ...
    def _get_access_token(self):
        ...
        try:
            ...
            logger.warning(f"fail to get access token current retry: {e}")


        raise Exception(
            f"fail to get access token , please check your network or ip"
        )
    ...
    def _login(self, access_token):
        ...
        raise Exception( f"fail to login after , please check your network or ip")

    def write_to_file(self, file_name, text):
        with open(file_name, "a", encoding="utf-8") as f:
            f.write(text)

    def _create_account(self, access_token, arkose,identifier):
        ...
        raise Exception(
            f"fail to create account , please check your network or ip"
        )
    ...
    def signup(self):
        ...
        is_verify_email = verify_email(self.sm,identifier)

        # task1 = verify_email()
        # task2 = self._get_access_token()
        # results = await asyncio.gather(task1, task2)

        if is_verify_email:
            # 邮件认证成功之后，账号就成功了，只是需要填写基本信息
            self.write_to_file("./data/credit.txt", f"\n{identifier}----{password}----")
            logger.info(f"account: {identifier} has verify email")
            ...
            result = self._create_account(access_token, arkose,identifier)
            ...
            if credit:
                logger.info(
                    f"account: {identifier} has credit: {credit['total_granted']}"
                )
                self.write_to_file(
                    "./data/credit.txt",
